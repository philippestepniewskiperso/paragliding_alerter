...
class TestSummitsAPI:
    ...
    def test_create(self, client):
        payload = {"name": "Chamechaude", "lat": 45.28, "lon": 5.77, "altitudes_m": [2000, 3000], "enabled": True}
    ...
    def test_update(self, client):
        r = client.post("/api/summits/", json={"name": "Old", "lat": 45.0, "lon": 5.0, "altitudes_m": [2000], "enabled": True})
        id = r.json()["id"]
        r = client.put(f"/api/summits/{id}", json={"name": "New", "lat": 46.0, "lon": 6.0, "altitudes_m": [3000], "enabled": False})
        assert r.status_code == 200
        assert r.json()["name"] == "New"

    def test_delete(self, client):
        r = client.post("/api/summits/", json={"name": "Del", "lat": 45.0, "lon": 5.0, "altitudes_m": [2000], "enabled": True})
        id = r.json()["id"]
    ...
    def test_toggle(self, client):
        r = client.post("/api/summits/", json={"name": "T", "lat": 45.0, "lon": 5.0, "altitudes_m": [2000], "enabled": True})
        id = r.json()["id"]
...
class TestSettingsAPI:
    ...
    def test_update(self, client):
        payload = {"ntfy_url": "https://ntfy.example.com", "ntfy_topic": "test", "wind_calm_kmh": 20.0, "cron_expression": "0 7 * * *"}
